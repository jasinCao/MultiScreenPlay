...
public class PlayerScaleView extends AutoRelativeLayout {

	private static final String TAG = "MovePlayerView";
	...
	public PlayerScaleView(Context context) {
		super(context);
		// TODO Auto-generated constructor stub
		init();
	}

	public PlayerScaleView(Context context, AttributeSet attrs) {
		super(context, attrs);
		// TODO Auto-generated constructor stub
		init();
	}

	public PlayerScaleView(Context context, AttributeSet attrs, int defStyle) {
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		backLayout = (ViewGroup) parentView.getChildAt(0);
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (event.getAction() == MotionEvent.ACTION_DOWN) {
			...
			// return true;
			// }
			Slog.d("ACTION_UP", "->" + (System.currentTimeMillis() - touchTime));
	...
	void startAlpha() {
		...
		if (parentView.getScrollX() == 0) {
			backLayout.setAlpha(1.0f);
		} else {
			backLayout.setAlpha(alphaValue + (float) parentView.getScrollX()
					/ (float) targetPointX);
		}
	}
